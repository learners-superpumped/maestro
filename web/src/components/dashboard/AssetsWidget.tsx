...
function AssetIcon({ type }: { type?: string }) {
  ...
  if (t.includes("image") || t.includes("photo"))
    return <Image className="h-4 w-4 text-[#9b9a97]" />;
  if (t.includes("chart") || t.includes("data"))
    return <BarChart2 className="h-4 w-4 text-[#9b9a97]" />;
  if (t.includes("text") || t.includes("doc"))
    return <FileText className="h-4 w-4 text-[#9b9a97]" />;
      ...
      <div className="mb-2">
        ...
        <div className="flex items-center gap-1 mt-0.5">
          ...
          <span className="text-[10px] text-[#9b9a97]">
            {driveConnected ? "Drive connected" : "Drive disconnected"} ·{" "}
            {assets.length} files
          </span>
        </div>
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-4 gap-1.5">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="aspect-square rounded bg-[#f7f6f3] animate-pulse"
            />
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-4 gap-1.5">
          {shown.map((asset) => (
            <button
              ...
              title={
                asset.title || asset.name || asset.asset_type || asset.type
              }
            >
          ...
          {shown.length === 0 && (
            <div className="col-span-4 py-2 text-[12px] text-[#9b9a97] px-1">
              No assets yet
            </div>
          )}
        </div>
