...
export function DashboardSidebar() {
  ...
  const approvals = (approvalsData?.approvals ?? []).map((a: Record<string, string>) => ({
    task_id: a.task_id,
    title: a.task_title ?? a.title ?? a.task_id,
    status: a.status,
    type: a.task_type ?? a.type,
  }));
  ...
  // Filter pending tasks that need manual start (approval_level >= 2)
  const pendingTasks = allTasks.filter(
    (t: Record<string, unknown>) => t.status === "pending" && ((t.approval_level as number) ?? 0) >= 2
  );
  ...
  for (const t of allTasks) {
    ...
    if (!tasksByGoalId[t.goal_id])
      tasksByGoalId[t.goal_id] = { total: 0, done: 0 };
    tasksByGoalId[t.goal_id].total++;
    if (t.status === "completed") tasksByGoalId[t.goal_id].done++;
  }

  const weekSpend: number[] = statsData?.week_spend_by_day ?? [
    0, 0, 0, 0, 0, 0, 0,
  ];
  ...
  return (
    <div className="w-[288px] shrink-0 border-l border-[#e8e5df] pl-5 space-y-5 overflow-y-auto">
      <ApprovalsWidget approvals={approvals} pendingTasks={pendingTasks} loading={approvalsLoading} />
