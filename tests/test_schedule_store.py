...
async def test_create_and_list_schedules(store):
    await store.create_schedule(
        name="daily-post", workspace="sns", task_type="create_post",
        cron="0 9 * * *", approval_level=2,
    )
    await store.create_schedule(
        name="engage", workspace="sns", task_type="engage",
        interval_ms=3600000, approval_level=0,
    )
...
async def test_get_schedule(store):
    await store.create_schedule(
        name="test", workspace="w", task_type="t", cron="* * * * *",
    )
...
async def test_update_schedule(store):
    await store.create_schedule(
        name="test", workspace="w", task_type="t", cron="0 9 * * *",
    )
...
async def test_list_enabled_schedules(store):
    await store.create_schedule(name="a", workspace="w", task_type="t", cron="* * * * *")
    await store.create_schedule(name="b", workspace="w", task_type="t2", cron="* * * * *")
